...
Image::ConstPtr V4l2CameraDevice::capture(opt_flow_msgs::opt_flow &flow)
{
  ...
  for (int i = 0; i < flow_cols; i = i + 8)
  {
    for (int j = 0; j < flow_rows; j++)
    {
      ...
      prev_point.x = (flow_mat.at<uint8_t>(j, i + 0) + flow_mat.at<uint8_t>(j, i + 1) * 256);
      prev_point.y = (flow_mat.at<uint8_t>(j, i + 2) + flow_mat.at<uint8_t>(j, i + 3) * 256);
      if (!(prev_point.x == 0 || prev_point.y == 0))
      {
        num_features++;
        // at(row,col)
        flow_data.x = flow_mat.at<int8_t>(j, i + 4);
        flow_data.y = flow_mat.at<int8_t>(j, i + 5);
